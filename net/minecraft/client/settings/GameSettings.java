        ...
        else
        {
            ...
            if (settingOption.getEnumFloat())
            {
                ...
                return settingOption == GameSettings.Options.MIPMAP_LEVELS && (double)f1 >= 4.0D ?
                		...
                		s1 + (int)f1) : (settingOption == GameSettings.Options.STREAM_FPS ? s1 + TwitchStream.formatStreamFps(f) + " fps" :
                		(settingOption == GameSettings.Options.STREAM_KBPS ? s1 + TwitchStream.formatStreamKbps(f) + " Kbps" :
                		(settingOption == GameSettings.Options.STREAM_BYTES_PER_PIXEL ? s1 + String.format("%.3f bpp",
                		new Object[] {TwitchStream.formatStreamBps(f)}): (f == 0.0F ? s1 + I18n.format("options.off") :
                		s1 + (int)(f * 100.0F) + "%"))))))))))))))));
            }
